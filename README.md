<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta http-equiv="X-UA-Compatible" content="IE=edge"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Heart Particles</title><script>var sdf = `
...
`;</script><script type="module">import * as THREE from "https://cdn.skypack.dev/three@0.136.0";
...
for (let i = 0; i < np; i++) {
let r = 100, R = 40;
let norm = i / np; // Norm increments from 0 to 1
let thetaVar = THREE.Math.randFloatSpread(0.5); // Random variation to theta [-0.5, 0.5]
let theta = norm * Math.PI * galaxySize + thetaVar;
let phi = THREE.Math.randFloatSpread(0.05); // Phi stays close to 0 to create galaxy ecliptic plane
let distance = norm * galaxySize; // Distance grows from 0 to galaxySize
...
const geometry = new TextGeometry('omg =))) ', {
font: font,
size: 5,
height: 5,
curveSegments: 10,
...
});</script><style>body,html{height:100%;margin:0;overflow:hidden;text-align:center}canvas{width:100%;height:100%;display:block}</style></head><body></body></html>
